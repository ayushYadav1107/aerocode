...
export const EnhancedCodeBlock: React.FC<EnhancedCodeBlockProps> = ({
  ...
  if (inline) {
    return (
      <code className="bg-muted/60 text-foreground px-1.5 py-0.5 rounded text-sm font-mono border border-border">
        {children}
      </code>
    )
  }
  ...
  return (
    <TooltipProvider>
      <div className={cn("relative group my-4", isFullscreen && "fixed inset-4 z-50 bg-background rounded-lg")}>
        {/* Header with enhanced controls */}
        <div className="flex items-center justify-between bg-muted/90 backdrop-blur-sm px-4 py-2.5 rounded-t-lg border border-border">
        ...
        {/* Code content */}
        <div className={cn(
          "border-x border-b border-border rounded-b-lg overflow-hidden relative",
          ...
          {collapsed && lineCount > 20 && (
            <div className={cn(
                "absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t to-transparent flex items-end justify-center pb-2",
                theme === "dark" ? "from-[#1e1e1e]" : "from-[#fafafa]",
              )}>
              <Button
                variant="ghost"
                size="sm"
