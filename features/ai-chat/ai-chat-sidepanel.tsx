...
const MessageTypeIndicator: React.FC<{
  ...
  const getTypeConfig = (type?: string) => {
    switch (type) {
      ...
      default:
        return { icon: MessageSquare, color: "text-muted-foreground", label: "Chat" };
    }
  };
...
const CodeSuggestionCard: React.FC<{
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden bg-muted/30 my-3 group hover:bg-card/50 transition-colors">
...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setInput("");
    setIsLoading(true);
    if (playgroundId) saveChatMessage(playgroundId, "user", newMessage.content);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (playgroundId) {
          saveChatMessage(playgroundId, "assistant", data.response);
        }
  ...
  return (
    <TooltipProvider>
      <>
        ...
        <div
          className={cn(
            "fixed right-0 top-0 h-full w-full max-w-6xl bg-background border-l border-border z-50 flex flex-col transition-transform duration-300 ease-out shadow-2xl",
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem
                      onClick={() => {
                        setMessages([]);
                        if (playgroundId) clearChatHistory(playgroundId);
                      }}
                    >
                      Clear All Messages
                    </DropdownMenuItem>
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-7 h-8 w-40 bg-muted/50 border-border"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 max-w-lg mx-auto">
                    {[
                      ...
                      <button
                        ...
                        className="px-3 py-2 bg-muted hover:bg-muted rounded-lg text-sm text-foreground transition-colors text-left"
                      >
                        {suggestion}
                      </button>
                    ))}
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        ...
                        msg.role === "user"
                          ? "bg-muted text-foreground p-4 rounded-br-md"
                        ...
                        <ReactMarkdown
                          ...
                          components={{
                            code: ({ children, className }) => (
                              <EnhancedCodeBlock
                                className={className}
                                // react-markdown v9 dropped the `inline` prop; a fenced
                                // block always has a trailing newline, inline code never does
            ...
            <div className="flex items-end gap-3">
              <div className="flex-1 relative">
                <Textarea
                  ...
                  disabled={isLoading}
                  className="min-h-[44px] max-h-32 bg-muted/50 border-border text-foreground placeholder:text-muted-foreground focus:border-blue-500 focus:ring-blue-500/20 resize-none pr-20"
                  rows={1}
                />
